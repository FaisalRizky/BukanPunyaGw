...
<div class="pemisah" style="border-bottom:2px #3a3a3a solid;">
	@foreach($gpu as $gpu)
          <h1 style="font-family:arvo; font-weight:bold;">{{$gpu->gpu_nama}}&nbsp;{{$gpu->gpu_series}}&nbsp;{{$gpu->gpu_tipe}}</h1>
...
<br>
            <div class="gambarhw" style="position:relative; left:0; max-height:800px; width:auto; border: 2px red solid;">
              <img style="width:100%;" itemprop="image" src="/gpu/{{$gpu->gpu_gambar}}">
			  </div>
      </div>
        <div class="col-md-9" >
          ...
          <div class="col-md-5" >
            <p style="background:#3a3a3a; border-radius:2px;">Nama GPU         </p>
            <p>Tipe GPU          </p>
            <p style="background:#3a3a3a; border-radius:2px;">Arsitektur       </p>
            <p>Core Speed</p>
            <p style="background:#3a3a3a; border-radius:2px;">Manufaktur        </p>
            <p>Memori Video          </p>
            <p style="background:#3a3a3a; border-radius:2px;">Tipe Memori</p>
            <p>Memori Bus           </p>
            <p style="background:#3a3a3a; border-radius:2px;">Kecepatan Memori</p>
            <p>Tanggal Rilis           </p>
          </div>
            <div class="col-md-1">
              <p>:</p>
            ...
            <div class="col-md-6">
              <center>
                <p>{{$gpu->gpu_nama}}&nbsp;{{$gpu->gpu_series}}</p>
                <p  style="background:#3a3a3a; border-radius:2px">{{$gpu->gpu_tipe}}</p>
                <p>{{$gpu->gpu_arch}}</p>
                <p  style="background:#3a3a3a; border-radius:2px">{{$gpu->gpu_speed}} - {{$gpu->gpu_boost}} (Boost)&nbsp;MHz</p>
                <p>{{$gpu->gpu_litografi}}&nbsp;nm</p>
                <p  style="background:#3a3a3a; border-radius:2px">{{$gpu->gpu_memory}}</p>
                <p>{{$gpu->gpu_memtype}}</p>
                <p  style="background:#3a3a3a; border-radius:2px">{{$gpu->gpu_membus}}</p>
                <p>{{$gpu->gpu_memspeed}}</p>
                <p style="background:#3a3a3a; border-radius:2px">{{$gpu->gpu_date}}</p>
              </center>
            </div>
    ...
    <div class="col-md-12 col-md-offset-3" style="padding-right:25%;">
      <br>
        <h1 style="font-family:Bahianita; margin-right:1.5%; font-weight: bold; background:#1a1a1a; border-radius:5px;">Ringkasan</h1>
          <p style="font-family:roboto mono; font-weight:200;">{{$gpu->gpu_nama}}&nbsp;{{$gpu->gpu_series}}&nbsp;{{$gpu->gpu_tipe}} Merupakan GPU yang memiliki kecepatan core
						{{$gpu->gpu_speed}} MHz dengan Boost maksimum {{$gpu->gpu_boost}} MHz. GPU ini dibekali Memori Video sebesar {{$gpu->gpu_memory}} GB
						Bertipe {{$gpu->gpu_memtype}} dan Bus Memori {{$gpu->gpu_membus}} bit dengan
						 kecepatan memorinya hingga {{$gpu->gpu_memspeed}} MHz. Teknologi Proses GPU ini adalah {{$gpu->gpu_litografi}}&nbsp;nm.
            </p>
          ...
          <h1 style="font-family:Bahianita; margin-right:1.5%; font-weight: bold; background:#00284d; border-radius:5px;">Berikan Ulasan dan Nilai Anda</h1>
            ...
            <form  method ="POST" action="review/submit" enctype="multipart/form-data">
        			{{ csrf_field() }}
							<input type="hidden" name="id" value="{{$gpu->id_gpu}}"/>
    ...
    <textarea class="form-control" rows="5" style="background:#f2f2f2;" name="isip" id="comment"></textarea>
          ...
          <input type="hidden" value="gpu" name="tipep"/>
...
<br>
  ...
  left: 50%;
	font-size:3vw;
  transform: translate(-50%, -50%);">{{$gpu->gpu_rating}}</h1>
	@endforeach
...
<h1 style="font-family:Roboto; font-weight:lighter; background:#00284d; border-radius:5px;">GPU Populer</h1>
<br>
<div class="popularposts">
	...
	<div class="thumbpagepop" title="{{$data -> gpu_nama}}">
				...
				<div class="snip" style="font-weight:lighter;">
					...
					<br>
					<?php
						echo strip_tags(str_limit("{$gpu->gpu_nama}&nbsp;{$gpu->gpu_series}&nbsp;{$gpu->gpu_tipe} Merupakan GPU yang memiliki kecepatan core
							{$gpu->gpu_speed} MHz dengan Boost maksimum {$gpu->gpu_boost} MHz. GPU ini dibekali Memori Video sebesar {$gpu->gpu_memory} GB
							Bertipe {$gpu->gpu_memtype} dan Bus Memori {$gpu->gpu_membus} bit dengan
							 kecepatan memorinya hingga {$gpu->gpu_memspeed} MHz. Teknologi Proses GPU ini adalah {$gpu->gpu_litografi}&nbsp; nm.", $limit = 299, $end = '...') ) ?>
					</div>
						<br>
						<br>
						<br>
...
<br>
		...
		<div class="footer">
<div class="botnavContainer" style="width:auto; position: relative; margin:0; margin-left: 9%; display: flex;">
<div class="logofoot">
