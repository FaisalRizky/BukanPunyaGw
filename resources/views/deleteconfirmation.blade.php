...
<div class="linknavigasi">
		...
		<div class="row">
          ...
          @foreach($news as $data)
					<p style="font-family:roboto;">Apa anda yakin ingin menghapus Post&nbsp;'<b>{{$data->judul}}</b>' ?</p>
          <form method="POST" action="/dashboard/deleteconfirmation/delete/{{$data->id}}" enctype="multipart/form-data">
            {{csrf_field()}}
            <input type="hidden" name="id" value="{{$data->id}}"/>
          <p><button type="submit" style="background-color: Transparent;
          background-repeat:no-repeat;
          border: none;
          cursor:pointer;
          overflow: hidden;
          outline:none;
          color:white;">Ya</button>&nbsp;&nbsp;</p><p><a href="{{url('/dashboard')}}">Tidak</a>
</form>
          </p>
@endforeach
			</div>
