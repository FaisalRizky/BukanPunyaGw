...
<div class="pemisah" style="border-bottom:2px #3a3a3a solid;">
	@foreach($cpu as $cpu)
          <h1 style="font-family:arvo; font-weight:bold;">{{$cpu->cpu_nama}}&nbsp;{{$cpu->cpu_series}}&nbsp;{{$cpu->cpu_tipe}}</h1>
...
<br>
            <div class="gambarhw" style="position:relative; left:0; max-height:800px; width:auto; border: 2px red solid;">
              <img style="width:100%;" itemprop="image" src="/cpu/{{$cpu->cpu_gambar}}">
			  </div>
      </div>
        <div class="col-md-9" >
          ...
          <div class="col-md-5" >
            <p style="background:#3a3a3a; border-radius:2px;">Nama CPU         </p>
            <p>Tipe CPU          </p>
            ...
            <div class="col-md-6">
              <center>
                <p>{{$cpu->cpu_nama}}&nbsp;{{$cpu->cpu_series}}</p>
                <p  style="background:#3a3a3a; border-radius:2px">{{$cpu->cpu_tipe}}</p>
                <p>{{$cpu->cpu_core}}</p>
                <p  style="background:#3a3a3a; border-radius:2px">{{$cpu->cpu_speed}}&nbsp;GHz</p>
                <p>{{$cpu->cpu_litografi}}&nbsp;nm</p>
                <p  style="background:#3a3a3a; border-radius:2px">{{$cpu->cpu_gpu}}</p>
                <p>{{$cpu->cpu_date}}</p>
                <p  style="background:#3a3a3a; border-radius:2px">{{$cpu->cpu_rammax}}</p>
              </center>
            </div>
    ...
    <div class="col-md-12 col-md-offset-3" style="padding-right:25%;">
      <br>
        <h1 style="font-family:Bahianita; margin-right:1.5%; font-weight: bold; background:#1a1a1a; border-radius:5px;">Ringkasan</h1>
          <p style="font-family:roboto mono; font-weight:200;">{{$cpu->cpu_nama}}&nbsp;{{$cpu->cpu_series}}&nbsp;{{$cpu->cpu_tipe}} Merupakan CPU yang memiliki {{$cpu->cpu_core}} CPU Core dengan
            kecepatan prosesor {{$cpu->cpu_speed}} GHz dan
          berteknologi proses sebesar {{$cpu->cpu_litografi}}&nbsp;nm, serta dibekali GPU {{$cpu->cpu_gpu}}. Prosesor ini dapat menampung Maksimum RAM Sebesar {{$cpu->cpu_rammax}} GB. </p>
    ...
    <textarea class="form-control" rows="5" style="background:#f2f2f2;" name="isip" id="comment"></textarea>
          ...
          <input type="hidden" value="cpu" name="tipep"/>
...
<br>
  ...
  left: 50%;
	font-size:3vw;
  transform: translate(-50%, -50%);">{{$cpu->cpu_rating}}</h1>
	@endforeach
...
<h1 style="font-family:Sofia; font-weight:lighter; background:#00284d; border-radius:5px;">CPU Populer</h1>
<br>
<div class="popularposts">
	@foreach($pops as $data)
	<a style="text-decoration:none;" href="{{ url('hardware/cpu/'.$data->id_cpu) }}">
	<div class="thumbpagepop" title="{{$data -> cpu_nama}}">
				<img src="/cpu/{{ $data -> cpu_gambar }}"><h3 style="color:#3498db;">{{$data->cpu_nama}}&nbsp;{{$data->cpu_series}}&nbsp;{{$data->cpu_tipe}}</h3></a>
				<div class="snip" style="font-weight:lighter;">
					<br>
					<br>
					<br>
					<?php
						echo strip_tags(str_limit("{$data->cpu_nama}&nbsp;{$data->cpu_series}&nbsp;{$data->cpu_tipe} Merupakan CPU yang memiliki {$data->cpu_core} CPU Core dengan kecepatan prosesor {$data->cpu_speed} GHz dan
	          berteknologi proses sebesar {$data->cpu_litografi}&nbsp;nm, serta dibekali GPU {$data->cpu_gpu}. Prosesor ini dapat menampung Maksimum RAM {$data->cpu_rammax}", $limit = 299, $end = '...') ) ?>
					</div>
						<br>
						<br>
						<br>
