		...
		<div class="linknavigasi">
			...
			@else
				...
				<div class="row">
		          ...
		          <center><p><h1 style="color:white; font-family:Verdana;">List CPU</h1></p></center>


							<div class="thumbpage" style="color:white; font-family:robotodraft;">
<br>
<h1><div class="stylings"><a href="\postcpu" style="font-family:robotodraft">Tambah Data (+)</a></div></h1>
    <center>
      <table id="table_id" style="color:white" class="display">
    <thead>
        <tr>

					<th class="column1"><center>ID cpu</th>
					<th class="column2"><center>Nama cpu</th>
					<th class="column3"><center>Kecepatan cpu</th>
					<th class="column4"><center>Core cpu</th>
					<th class="column5"><center>Thumbnail cpu</th>
					<th class="column6"><center>Rating cpu</th>
					<th class="column7"><center>Manufaktur cpu</th>
					<th class="column6"><center>Maksimum RAM</th>
    ...
    <tbody>
			@foreach($cpu as $cpu)
				<tr style="color:black; text-align:center;">
					<td class="column1">{{$cpu->id_cpu}}</td>
					<td class="column2">{{$cpu->cpu_nama}}&nbsp;{{$cpu->cpu_series}}&nbsp;{{$cpu->cpu_tipe}}</td>
					<td class="column3">{{$cpu->cpu_speed}} GHz</td>
					<td class="column4">{{$cpu->cpu_core}}</td>
					<td class="column5"><div class="styling"><a href="{{url('/cpu/'.$cpu->cpu_gambar)}}">{{$cpu->cpu_gambar}}</a></div></td>
					<td class="column6">{{$cpu->cpu_rating}}</td>
					<td class="column7">{{$cpu->cpu_litografi}} nm</td>
					<td class="column7">{{$cpu->cpu_rammax}} GB</td>
					<td class="column8">{{$cpu->penulis}}</td>
					<th class="column9"><div class="styling"  style="color:white;"><a href="hardware/soc/{{$cpu -> id_cpu}}" target="_blank"><center><p><a href="editcpu/{{$cpu -> id_cpu}}">Ubah</a></p>
						<p><button style="color:black;" data-id="{{ $cpu->id_cpu }}" class="hapus">Hapus</a></p></div></th>
				</tr>
@endforeach
    </tbody>
